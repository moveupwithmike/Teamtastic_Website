...
1. Generates a 60-90 second plain-English spoken script via an Anthropic model (`anthropic/claude-haiku-4.5`) called through Vercel AI Gateway's Anthropic-Messages-shaped REST endpoint (`https://ai-gateway.vercel.sh/v1/messages`), instructed to state only what's in the data and call out gaps/staleness rather than invent anything.
2. Converts that script to audio via AI Gateway's speech REST endpoint (`https://ai-gateway.vercel.sh/v4/ai/speech-model`, header `ai-model-id: openai/tts-1` — the REST path only supports OpenAI speech models), which returns base64-encoded MP3.
