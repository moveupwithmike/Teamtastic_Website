...
## Conditional-Go Remediation

A follow-on engagement closed all four named conditional-go findings. Full detail, evidence, and the final verdict live in [`TEAMTASTIC_COMMERCIAL_GO_CLOSURE.md`](./TEAMTASTIC_COMMERCIAL_GO_CLOSURE.md); this section summarizes the disposition of each finding against this rehearsal's own numbering.

| Rehearsal finding | Disposition | Evidence |
|---|---|---|
| P0 #2 — live Stripe never exercised end-to-end | **Closed via isolated test-mode certification** | A dedicated Vercel preview branch (`stripe-test-certification`) was built with branch-scoped Stripe TEST keys and a real `stripe listen` webhook forward — real application code (the actual `/api/stripe/checkout` and `/api/stripe/webhook` routes), not unit tests. Proved: two successful payments, duplicate webhook delivery handled idempotently, a declined payment, one single refund, two sequential partial refunds on the same payment, a full refund, and payment-expiry recovery. Production Stripe env vars confirmed unchanged (still the same values from before this branch existed) — no test key ever reached production, and production remains live mode by design. See closure doc §3–6. |
| P1 #3 — no unique constraint on `leads.submission_id`, dedupe race condition | **Corrected as a false positive, not fixed** | Re-investigation found `CREATE UNIQUE INDEX leads_submission_id_key` already exists and already enforces uniqueness at the database level (the original finding only checked `pg_constraint`, which does not list indexes created via `CREATE UNIQUE INDEX` rather than `ADD CONSTRAINT`). Reproduced directly: a second insert with a duplicate `submission_id` fails with Postgres error `23505`, exactly as the app's existing `error?.code === "23505"` handling in `src/app/api/leads/route.js` expects. Two pre-existing Vitest tests already cover this scenario. A production audit found 0 duplicate `submission_id` values among current leads. No schema change was made. See closure doc §7–8. |
| P2 #4 — `/office/prospects` does not exclude synthetic/QA records | **Fixed** | [`src/app/office/(private)/prospects/page.js`](./src/app/office/(private)/prospects/page.js) now excludes prospects whose linked lead carries `context.synthetic_test === true`, using the same canonical boundary `office/(private)/page.js` already used — no page-local regex or bespoke rule. All other office dashboards were audited; only the main dashboard, prospects, and the ROI RPC present commercially-aggregated lead/prospect data, and all three now apply the same rule consistently. See closure doc §9. |
| P2 #5 — UTM/attribution lost on multi-page browsing before conversion | **Fixed** | [`src/lib/lead-client.js`](./src/lib/lead-client.js) now captures first-touch UTM parameters into `sessionStorage` the moment they're first seen (via `captureFirstTouchAttribution()`, wired into the already-globally-mounted `FunnelIntentTracker`) and preserves them across internal navigation instead of re-reading `window.location.search` fresh at submit time. Session-scoped, first-party, no PII, no fingerprinting, no persistent cookie. Verified with 6 new regression tests and live on production (`teamtastic.events`) via safe browser navigation with no form submission. See closure doc §10. |

Finding #6 (Turnstile loading-state fallback) and the standing `phase4_lifecycle_enabled = false` posture (paid deposits do not yet auto-convert to `deals`) were confirmed unchanged and are carried forward as documented, non-blocking operational facts — see the closure doc's Remaining Limitations section.

**Updated verdict: GO — COMMERCIAL SALES ENGINE READY.** See `TEAMTASTIC_COMMERCIAL_GO_CLOSURE.md` for the full record.
