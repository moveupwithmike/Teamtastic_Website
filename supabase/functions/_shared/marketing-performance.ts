// Extracted from sync-marketing-performance/index.ts (pure move, no behavior
// change) so tests can exercise each platform's sync path -- including
// missing-credentials and provider-error failure modes -- with a mocked
// fetch, without triggering Deno.serve() at module load. Mirrors the same
// extraction already done for _shared/gmail-classification.ts and
// _shared/voice-brief.ts.
...
export async function getGoogleMarketingAccessToken(): Promise<string> {
...
export async function syncGoogleAnalytics(): Promise<Record<string, unknown> | null> {
...
export async function syncSearchConsole(): Promise<Record<string, unknown> | null> {
...
export async function syncGoogleAds(): Promise<Record<string, unknown> | null> {
...
export async function syncMetaAds(): Promise<Record<string, unknown> | null> {
...
export const PLATFORM_SYNCERS: { platform: string; sync: () => Promise<Record<string, unknown> | null> }[] = [
