  ...
  return (
    <header
      className={`sticky top-0 z-50 w-full transition-all duration-300 ${
        isFamilyPage
          ? "bg-zinc-950/95 border-b border-white/5 py-3 shadow-md"
    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          {/* Desktop Navigation - Hidden on landing page for optimized conversion */}
          {!isExperiencesPage && (
            <nav className="hidden lg:flex items-center gap-6">
              {(isFamilyPage ? familyNavLinks : navLinks).map(({ href, label, icon: Icon, iconColor }) => (
                <Link
                  key={href}
                  href={href}
          ...
          <div className="flex items-center gap-4">
            {!isExperiencesPage && !isFamilyPage && (
              <a
                href="https://teamtastic.games"
                className="hidden lg:flex text-sm font-medium text-zinc-300 hover:text-white transition-colors flex items-center gap-1.5 px-3 py-1.5 rounded-lg hover:bg-white/5"
              >
            ...
            {isFamilyPage ? (
              <button
                onClick={() => window.dispatchEvent(new CustomEvent("open-family-concierge"))}
                className="inline-flex items-center justify-center px-5 py-2.5 rounded-full text-xs font-bold uppercase tracking-wider text-white bg-emerald-600 hover:bg-emerald-500 hover:scale-[1.02] shadow-[0_0_15px_rgba(16,185,129,0.3)] transition-all duration-300 cursor-pointer"
              >
          ...
          {/* Mobile Menu Button - Hidden on experiences page */}
          {!isExperiencesPage && (
      ...
      {/* Mobile Drawer - Hidden on experiences page */}
      {!isExperiencesPage && isOpen && (
          ...
          <div className="flex flex-col gap-3">
            ...
            {isFamilyPage ? (
              <button
                onClick={() => {
                  setIsOpen(false);
                  window.dispatchEvent(new CustomEvent("open-family-concierge"));
                }}
                className="w-full flex h-11 items-center justify-center gap-2 rounded-xl text-xs font-bold uppercase tracking-wider text-white bg-emerald-600 hover:bg-emerald-500 transition-colors cursor-pointer"
              >
