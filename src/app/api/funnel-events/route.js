...
export async function POST(request) {
  ...
  const rateKey=createHash("sha256").update(`funnel:${ip}:${sessionId}`).digest("hex");
  if (rateLimited(rateKey,{ windowMs:60_000,max:60 })) return NextResponse.json({ success:false },{ status:429 });
