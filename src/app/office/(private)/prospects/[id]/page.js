...
export default async function ProspectDetail({ params }) {
  ...
  const [companyResult, leadsResult, messagesResult, bookingsResult, tasksResult, dealsResult, logsResult, draftsResult, proposalsResult] = await Promise.all([
    ...
    db.from("leads").select("id,name,email,company,phone,lead_source,status,team_size,vibe,occasion,recommendation_key,preferred_event_date,alternate_event_date,event_timezone,preferred_time,budget_range,package_interest,decision_timeline,utm_source,utm_campaign,context,created_at").eq("prospect_id", id).order("created_at", { ascending: false }),
